...
class TestPointClickCarePatient_CreateEncounter:

    def test_create_encounter_for_matrixcare_patient(self):
        ...
        #patientspage.verify_patient_page_loaded()
        patients_page.search_pointclickcarepatient_and_navigate_to_patientdetails_page()

        patient_details_page=PatientDetailsPage(self.driver)
        patient_name=patient_details_page.get_patient_name()
        print(f"Patient Name: {patient_name[0]} {patient_name[1]}")
        #assert patientdetails_page.verify_patient_details_page_loaded(), "Patient Details page did not load successfully."
